...
const apStart = content.indexOf('<AnimatePresence mode="wait">');
const mainEnd = content.indexOf('</main>', apStart);
const apEnd = content.lastIndexOf('</AnimatePresence>', mainEnd) + '</AnimatePresence>'.length;

if (apStart !== -1 && mainEnd !== -1) {
  content = content.slice(0, apStart) + viewsJSX.trim() + '\n      ' + content.slice(apEnd);
}

fs.writeFileSync(appTsxPath, content);
console.log('App.tsx accurately updated');
